...
> than marking it "done" here. Tiers 1 and 2 are both fully done as of 2026-08-09/10 — see `git log`.
> Everything below is renumbered accordingly, starting fresh at Tier 3.

## Tier 3 — Efficiency on what's already built

1. **Optimize `/roster/build` layout** — the user's own framing: "functionality good, layout maybe not."
...
2. **Login page → home page → rest of the app.** Login and permission levels (Tier 1) are both done — a
...
3. **Side nav instead of the current top nav bar** — natural to pair with #2, since both are about overall
   app navigation/IA. Worth doing together rather than as two separate passes.

## Tier 5 — Growth / polish

4. **Reporting: report builder.** Already flagged as a deliberate "nice to have, later" when phase 5 shipped
...
5. **Style/branding settings** — company colour codes + logo, logo shown in the currently-empty right side
...
6. **Test framework for code & UI testing.** Nothing's set up yet — this whole build has been verified via
...
7. **User manual / help function.** Lowest priority — most valuable once the feature set and UI settle down
